...
class AnalyzeTech {
  ...
  def static parseNewTech(Set<String> data) {
    ...
    if (data.contains(upnp)) {
      PrintUtils.printMarkdownMessage(
              "新技术：UPnP 是各种各样的智能设备、无线设备和个人电脑等实现遍布全球的对等网络连接（P2P）的结构。UPnP 是一种分布式的，开放的网络架构。UPnP 是独立的媒介。")
    }
    ...
    if (data.contains(react)) {
      PrintUtils.printMarkdownMessage("新技术：facebook--react native")
    }

    if (data.contains(yoga)) {
      PrintUtils.printMarkdownMessage("新技术：facebook开源跨平台前端布局引擎Yoga")
    }
  ...
  def static parseOthers(Set<String> data) {
    ...
    if (data.contains(butterknife)) {
      PrintUtils.printMarkdownMessage("others：butterknife ")
    }

    if (data.contains(uuid)) {
      PrintUtils.printMarkdownMessage("others：uui-com.eaio.util ")
    }

    if (data.contains(un7z)) {
      PrintUtils.printMarkdownMessage("others：7z解压")
    }
